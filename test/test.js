...
describe('postcss-default-unit', function () {
  ...
  it('adds specified unit', function () {
    test('i {font-size: 1.5}', 'i {font-size: 1.5em}', { unit: 'em' });
  });

  it('does not add to non-numeric values', function () {
    test('a {property: 10 foo-10 10-foo -10 10-}', 'a {property: 10px foo-10 10-foo -10px 10-}');
  });
  ...
  it('leaves all expressions in parentheses untouched for now', function () {
    test(
      'p { border: 2 dashed rgba( 255 , 64 , 64 , 0.3 ); }',
  ...
  it('processes comma-separated values', function () {
    test(
      '.bg { background-size: 10 2 ,35 4,35 4, 91 61;}',
  ...
  it('adds default unit to media query', function () {
    test(
      '@media screen and (max-width: 980) { #main {width: 95%; } }',
  ...
  it('adds units to several params in a query', function () {
    test(
      '@media screen and (max-width: 980) and (min-width: 480) { }',
